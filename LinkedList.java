...
public class LinkedList {
    ...
    public void addFirst(MemoryBlock block) {
        ...
        if (first == null) { // If the list is empty
            first = newNode;
            last = newNode; // `last` must point to the new node as well
        } else {
            newNode.next = first; // Point the new node to the current first
    ...
    public void remove(Node node) {
        if (node == null || first == null) {
            throw new NullPointerException(
                    "cannot remove null node or the list is empty");
        }
        Node prev = null;
        Node current = first;
